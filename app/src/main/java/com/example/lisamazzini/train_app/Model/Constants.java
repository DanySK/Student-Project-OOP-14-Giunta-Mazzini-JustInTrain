package com.example.lisamazzini.train_app.Model;

/**
 * Contains constants for the whole project.
 *
 * @author Alberto Giunta
 */

public class Constants {

    public static final int N_TIME_SLOT = 5;

    public static final String JOURNEY_PREF_FILE = "JourneyPref";
    public static final String TRAIN_PREF_FILE = "TrainPref";

    public static final String BASE_URL = "http://mobile.viaggiatreno.it";
    public static final String EXT_URL= "/vt_pax_internet/mobile";
    public static final String ACTION_PLANNED = "/programmato";
    public static final String ACTION_NUMBER = "/numero";
    public static final String NOT_DEPARTED_YET = "partito";
    public static final String ON_TIME = "orario";
    public static final String LATE = "ritardo";
    public static final String IN_ADVANCE = "anticipo";
    public static final String IS_MOVING = "viaggia";
    public static final String ALREADY_VISITED = "giaeffettuate";
    public static final String NOT_VISITED_YET = "corpocentrale";


}
